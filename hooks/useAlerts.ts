...
import { fetchAlerts, validAlerts } from '../services/alertsService';
...
export function useAlerts(center: Coordinate | null, foreground: boolean) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (config.demo) { setStatus('live'); setAlerts(previous => validAlerts(previous)); }
        ...
        try {
          const result = await fetchAlerts(centerRef.current, controller.signal);
          if (!cancelled) { setAlerts(result); setStatus('live'); }
        } catch { if (!cancelled) { setStatus('offline'); setAlerts(previous => validAlerts(previous)); } }
      }
  ...
  return { alerts, status, add: (alert: RoadAlert) => setAlerts(previous => validAlerts([...previous, alert])) };
}
